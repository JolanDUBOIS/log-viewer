<script>
  ...
  import { logColumns } from '../../stores/logsStore.js';
  import { columnsMeta, updateAndSaveColumnMeta } from '../../stores/columnsStore.js';
  import { sortingColumn, updateSortingColumn } from '../../stores/sortingStore.js';
  ...
  function toggleVisibility(columnKey) {
    const col = get(columnsMeta)[columnKey];
    if (!col) return;
    updateAndSaveColumnMeta({ ...col, visible: !col.visible });
  }
  ...
  function handleInputKeydown(event, colKey) {
    if (event.key === 'Enter') {
      saveAlias(colKey, event.target.value);
      colEditMode = null;
    }
  }
  ...
  function saveAlias(colKey, newAlias) {
    const col = get(columnsMeta)[colKey];
    if (!col) return;
    updateAndSaveColumnMeta({ ...col, alias: newAlias });
  }
</script>
  ...
  {#each $logColumns as colKey}
    <div class="column-item">
      ...
      <div class="alias-container">
        {#if colKey === colEditMode}
          <input 
            type="text" 
            class="alias-input" 
            value={$columnsMeta[colKey].alias} 
          ...
          />
        {:else}
          <span>{$columnsMeta[colKey].alias}</span>
        {/if}
      </div>
      ...
      <button 
        ...
        on:click={() => $sortingColumn !== colKey && updateSortingColumn(colKey)}
        aria-label="Toggle order by" 
        title="Toggle order by"
        class:disabled={$sortingColumn === colKey}
      >
