<script>
  ...
  import { history, addToHistory, deleteHistoryItem } from '../../stores/historyStore.js';
  import { selectActiveFile } from '../../stores/activeFileStore.js';
  import { loadLogs } from '../../stores/logsStore.js';
  ...
  async function clickFile(fileAlias) {
    ...
    const entry = currentHistory.find(item => item.alias === fileAlias);
    if (!entry) {
      console.error('File alias not found in history:', fileAlias);
      return;
    }

    await selectActiveFile(entry);
    await loadLogs();
  }

  async function removeFile(fileAlias) {
    console.log('Removing file:', fileAlias);
    let filePath = get(history).find(item => item.alias === fileAlias)?.path;
  ...
  function saveFileAlias(fileAlias, newAlias) {
    const currentHistory = get(history);
    const entry = currentHistory.find(item => item.alias === fileAlias);
    const newEntry = { ...entry, alias: newAlias };
...
<div class="log-files-list">
  {#each $history as fileAlias}
    <div class="column-path-item">
      ...
      <button class="delete-button" on:click={() => removeFile(fileAlias)} aria-label="Remove File from History" title="Remove File from History">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-6">
