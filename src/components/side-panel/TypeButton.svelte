<script>
  import { typeDropdown, toggleTypeDropdown, openTypeDropdown, closeTypeDropdown } from '../../stores/uiStore.js';
  import { columnsMeta, updateAndSaveColumnMeta } from '../../stores/columnsStore.js';

  export let columnKey;

  async function setType(type) {
    const col = $columnsMeta[columnKey];
    if (!col) return;

    await updateAndSaveColumnMeta({ ...col, type });
    closeTypeDropdown();
  }
</script>
...
<div class="type-button-wrapper">
  <button
    class="type-button"
    on:click={() => toggleTypeDropdown(columnKey)}
    aria-label="Choose column type"
    title="Choose column type"
  >
    {#if $columnsMeta[columnKey].type === 'number'}
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
        <path stroke-linecap="round" stroke-linejoin="round" d="M5.25 8.25h15m-16.5 7.5h15m-1.8-13.5-3.9 19.5m-2.1-19.5-3.9 19.5" />
      </svg>
    {:else if $columnsMeta[columnKey].type === 'datetime'}
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
    ...
    {:else if $columnsMeta[columnKey].type === 'category'}
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
  ...
  {#if $typeDropdown === columnKey}
    <div
      role="region"
      class="type-dropdown"
      on:mouseenter={() => openTypeDropdown(columnKey)}
      on:mouseleave={() => closeTypeDropdown()}
    >
